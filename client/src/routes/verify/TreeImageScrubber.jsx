    ...
    const handleScroll = () => {
      if (
        ...
        Math.floor(scrollContainerRef.scrollTop) !==
          Math.floor(scrollContainerRef.scrollHeight) - Math.floor(scrollContainerRef.offsetHeight)
      ) {
        return
      }
      props.verityDispatch.loadMoreTreeImages()
  ...
  async function handleSubmit(approveAction) {
    ...
    if (isNew) {
      const answer = await new Promise((resolve) => {
        if (
          window.confirm(`The species ${props.speciesState.speciesInput} is a new one, create it?`)
        ) {
          resolve(true)
        } else {
          resolve(false)
    ...
    if (result) {
      ...
      if (props.verityState.treeImagesSelected.length === props.verityState.treeImages.length) {
  ...
  return (
    <React.Fragment>
      ...
      {props.verityState.isApproveAllProcessing && (
        ...
        >
          <LinearProgress color="primary" variant="determinate" value={complete} />
        </AppBar>
      )}
      {props.verityState.isApproveAllProcessing && (
        <Modal open={true}>
          ...
          <Grid container justify="space-between">
            <Grid item>
              <Typography variant="body2" color="primary" gutterBottom>
                Tree #{dialog.tree.id}, Planter #{dialog.tree.planterId}, Device #
                {dialog.tree.deviceId}
              </Typography>
              <Typography variant="body2" color="primary" gutterBottom>
